...
def download_monthly_indexPriceKlines(
    trading_type,
    symbols,
    num_symbols,
    intervals,
...
def download_daily_indexPriceKlines(
    trading_type,
    symbols,
    num_symbols,
    intervals,
    ...
    for symbol in symbols:
        print(
            "[{}/{}] - start download daily {} klines ".format(
                current + 1, num_symbols, symbol
            )
        )
        for interval in intervals:
            for date in dates:
                ...
                if start_date <= current_date <= end_date:
                    path = get_path(
                        trading_type, "indexPriceKlines", "daily", symbol, interval
                    )
...
if __name__ == "__main__":
    ...
    if args.dates:
        ...
        download_monthly_indexPriceKlines(
            args.type,
            symbols,
            num_symbols,
            args.intervals,
