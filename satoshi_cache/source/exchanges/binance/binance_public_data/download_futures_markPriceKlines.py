...
def download_monthly_markPriceKlines(
    trading_type,
    symbols,
    num_symbols,
    intervals,
    ...
    for symbol in symbols:
        print(
            "[{}/{}] - start download monthly {} markPriceKlines ".format(
            ...
            for year in years:
                for month in months:
                    ...
                    if start_date <= current_date <= end_date:
                        path = get_path(
                            trading_type, "markPriceKlines", "monthly", symbol, interval
                        )
...
def download_daily_markPriceKlines(
    trading_type,
    symbols,
    num_symbols,
    intervals,
    ...
    for symbol in symbols:
        print(
            "[{}/{}] - start download daily {} markPriceKlines ".format(
                current + 1, num_symbols, symbol
            )
        )
        for interval in intervals:
            for date in dates:
                ...
                if start_date <= current_date <= end_date:
                    path = get_path(
                        trading_type, "markPriceKlines", "daily", symbol, interval
                    )
...
if __name__ == "__main__":
    ...
    if args.dates:
        ...
        download_monthly_markPriceKlines(
            args.type,
            symbols,
            num_symbols,
            args.intervals,
